const genreData = {
  ...
  const moodToGenresMapping = {
    "happy": ["acoustic", "bossanova", "disco", "funk", "salsa"],
    "angry": ["death-metal", "hard-rock", "metal", "punk"],
    "surprised": ["anime", "chill", "comedy", "disney"],
    "sad": ["blues", "country", "folk", "gospel", "jazz"]
  };
  ...
  genreData.genres.forEach((genre) => {
    for (const mood in moodToGenresMapping) {
      if (moodToGenresMapping[mood].includes(genre)) {
        genresByMood[mood].push(genre);
      }
    }
  });
  
  console.log("Genres by Mood:");
  console.log(genresByMood);
